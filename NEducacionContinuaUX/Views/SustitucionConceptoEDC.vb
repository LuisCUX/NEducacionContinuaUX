Public Class SustitucionConceptoEDC
    ...
    Private Sub SustitucionConceptoEDC_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Dim tableObservacion As DataTable = db.getDataTableFromSQL($"SELECT ID, Observacion FROM ing_CatObservacionesCancelacion WHERE Activo = 1 AND ID_TipoOtraObservacion = 2")
    ...
    Private Sub btnBuscar_Click(sender As Object, e As EventArgs) Handles btnBuscar.Click
        ...
        sd.LlenarGridConceptosSC(IDFolio, GridConceptos)
        ...
        For x = 0 To GridConceptos.Rows.Count - 1
            total = total + Convert.ToDecimal(GridConceptos.Rows(x).Cells(8).Value)
        Next
        lblTotal.Text = total
    End Sub
    ...
    Sub Reiniciar()
        Me.Controls.Clear()
        InitializeComponent()
        SustitucionConceptoEDC_Load(Me, Nothing)
    End Sub
    ...
    Private Sub btnGuardarSustitucion_Click(sender As Object, e As EventArgs) Handles btnGuardarSustitucion.Click
        ...
        For x = 0 To GridConceptos.Rows.Count - 1
            If (GridConceptos.Rows(x).Cells(2).Value.ToString() = GridConceptos.Rows(x).Cells(7).Value.ToString()) Then
                Dim result As DialogResult = MessageBox.Show("El nombre de uno o mas conceptos es igual que el nombre con el que fue timbrado ¿Quiere continuar con la sustitución?", "", MessageBoxButtons.YesNo)
                If (result <> 6) Then
                    Me.Reiniciar()
                    Exit Sub
                End If
        ...
        For x = 0 To GridConceptos.Rows.Count - 1
            listaconceptosFinal(x).NombreConcepto = GridConceptos.Rows(x).Cells(7).Value
