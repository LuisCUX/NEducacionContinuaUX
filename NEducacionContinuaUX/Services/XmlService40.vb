...
Public Class XmlService40
    ...
    ''------------------------------------------CADENA PAGO NORMAL------------------------------------------''
    ''------------------------------------------------------------------------------------------------------------------------------''
    Function cadenaSustitucion(Serie As String, Folio As String, Fecha As String, FormaPago As String, NoCertificado As String, SubTotal As String, Descuento As String, Total As String, listaConceptos As List(Of Concepto), totalIVA As String, RFC As String, NombreCompleto As String, Credito As Boolean, CP As String, RegFiscal As String, CostoBaseTotal As String, usoCFDI As String, UUIDSustituido As String) As String
        Dim metodoPago As String
        If (Credito = True) Then
            metodoPago = "PPD"
        Else
        ...
        Dim bandIVA As Boolean = False
        Dim cadena As String = "||4.0|" & Serie & "|" & Folio & "|" & Fecha & "|" & FormaPago & "|" & NoCertificado & "|" & SubTotal.ToString() & "" & Descuentotxt & "MXN|" & Total & "|I|01|" & metodoPago & "|" & cpemisor & "|04|" & UUIDSustituido & "|" & EnviromentService.RFCEDC & "|" & nombreEmisor & "|" & EnviromentService.RegimenFiscal & "|" & RFC & "|" & NombreCompleto & "|" & cpreceptor & "|" & RegFiscal & "|" & usoCFDI & ""
        For Each concepto As Concepto In listaConceptos
            If (concepto.descuento <> 0) Then
                Descuentotxt = $"|{Descuento.ToString()}"
            Else
    ...
    ''------------------------------------------XML PAGO NORMAL------------------------------------------''
    ''------------------------------------------------------------------------------------------------------------------------------''
    Function xmlPruebaSustitucion(Total As String, SubTotal As String, Descuento As String, TotalIVA As String, Fecha As String, Sello As String, Certificado As String, NoCertificado As String, FormaPago As String, Folio As String, Serie As String, UsoCFDI As String, listaConceptos As List(Of Concepto), RFC As String, NombreCompleto As String, Credito As Boolean, Cp As String, RegFiscal As String, CostoIVABaseTotal As String, UUIDSustituido As String) As String
        Dim IVABand As Boolean = False
        Dim config As New XmlWriterSettings
        config.Indent = True
        config.Encoding = Encoding.UTF8
        ...
        Using sw As New StringWriter()
            Using wr As XmlWriter = XmlWriter.Create(sw, config)
                ...
                wr.WriteStartElement("cfdi", "CfdiRelacionados", Nothing) ''NODO CFDI RELACIONADOS START
                wr.WriteAttributeString("TipoRelacion", Nothing, "04")
                wr.WriteStartElement("cfdi", "CfdiRelacionado", Nothing)
                wr.WriteAttributeString("UUID", Nothing, UUIDSustituido)
                wr.WriteEndElement()
                wr.WriteEndElement()
